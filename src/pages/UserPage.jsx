...
const getLevelProgress = profile => {
...
const UserPage = ({ userId, token }) => {
	...
	if (!profile) {
		return (
			<div className='bg-gray-900 min-h-screen flex items-center justify-center'>
				<p className='text-gray-400 text-xl font-semibold'>Профиль не найден</p>
			</div>
		);
	}
	...
	const currentLevelName = profile.level
		...
		: 'Новичок';
	const { progress, nextLevel } = getLevelProgress(profile);
	...
	return (
		...
		>
			<div className='max-w-5xl mx-auto'>
				<button
					onClick={() => navigate(-1)}
					className='flex items-center text-cyan-400 hover:text-cyan-300 transition-all duration-300 mb-6 text-lg font-medium'
				>
					<svg
				...
				<h2 className='text-4xl font-bold text-white mb-8 text-center'>
					Профиль {profile.role === 'Freelancer' ? 'фрилансера' : 'клиента'}
				</h2>
				{error && (
					<motion.p
						initial={{ opacity: 0 }}
				...
				<div className='bg-gray-800/50 backdrop-blur-md rounded-xl shadow-lg p-6 sm:p-8'>
					<div className='flex flex-col md:flex-row gap-8'>
						<div className='flex-1'>
							...
							<p className='text-gray-400'>{profile.email || 'Нет email'}</p>
								...
								{isFavoritesLoading ? (
									...
									<FavoriteButton
										...
										isFavorited={favorites.some(
											fav =>
												fav.freelancerId?.toLowerCase() === id?.toLowerCase()
										)}
										onToggle={() =>
											setFavorites(prev => {
												...
												if (exists) {
													...
													return [...prev, { freelancerId: id, teamId: null }];
												}
											})
										}
									/>
