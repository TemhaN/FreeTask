...
import { register, verifyEmailLink, API_BASE_URL } from '../api/api';
import VerifyEmail from '../components/Auth/VerifyEmail';

const RegisterPage = ({ setToken, setUserId, setIsAdmin }) => {
	...
	const handleSubmit = async e => {
		...
		try {
			...
			if (res.data.accessToken) {
				if (res.data.message?.includes('Код подтверждения')) {
			...
			<div className='max-w-md w-full bg-gray-800 rounded-xl shadow-lg p-6 sm:p-8'>
				...
				{showVerify ? (
					...
					<div className='space-y-6'>
						<div>
							<label className='block text-white font-medium mb-2'>Роль</label>
							<select
								value={role}
								onChange={e => setRole(e.target.value)}
								className='w-full px-4 py-3 bg-gray-700 text-white rounded-lg border border-gray-600 focus:outline-none focus:ring-2 focus:ring-cyan-500 transition-all duration-300'
							>
						...
						<motion.button
							...
							whileHover={{ scale: 1.05 }}
							whileTap={{ scale: 0.95 }}
							className='w-full bg-gray-600 text-white px-6 py-3 rounded-lg font-semibold hover:bg-gray-500 transition-all duration-300'
						>
					...
					<div className='space-y-6'>
						...
						<motion.button
							...
							whileHover={{ scale: 1.05 }}
							whileTap={{ scale: 0.95 }}
							className='flex items-center justify-center w-full px-6 py-3 bg-white text-gray-700 rounded-lg border border-gray-300 font-semibold hover:bg-gray-100 hover:shadow-lg transition-all duration-300 focus:ring-2 focus:ring-blue-500'
						>
							<svg
